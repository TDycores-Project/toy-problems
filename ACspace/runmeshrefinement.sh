...
echo ",mesh_res,u_error,p_error" > conv_test_result.csv
i=0
...
for ((res=${test_flags[res_start]}; res<=${test_flags[res_end]}; res+=${test_flags[res_stride]})); do
    ...
    python3 FEAC1red2D.py $args | grep "Velocity and Pressure Absolute Error:" | awk -v i="$i" -v res="$res" '{ print i","res","$6","$7}' >> conv_test_result.csv
    i=$((i+1))
done
