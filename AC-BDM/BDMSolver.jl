...
# Run with: 
#          julia BDMSolver.jl
#          julia BDMSolver.jl --mesh random --aspect_ratio 5 --N 4
#          julia BDMSolver.jl --mesh random --aspect_ratio 2 --N 3 --plot_mesh
#
...
#                        Basis, and DivBasis
#----------------------------------------------------
function GetNodalBasis(xhat, yhat)
  ...
  v7|         |v3
    1---------2
     v1     v2
    """
    # see my repository to see how this function is generated
    m = length(xhat)
    Bx = zeros(m,8)
    By = zeros(m,8) 
...
function GetDivNodalBasis()
    """This function returns the divergence of Nodal basis
    
    Output:
    -------
    Dhat: the divergence of nodal basis
    shape (1,8)
    """
    Dhat = [0.25 0.25 0.25 0.25 0.25 0.25 0.25 0.25]
...
function GetLocalOperator(Coord_E, Q1d, Forcing)
    ...
    for i=1:m
        ...
        kinv = inv(k)
        JJ = J[:,i,:]
        W[2*i-1:2*i, 2*i-1:2*i]=JJ' * kinv * JJ * w[i]/detJ[i] 
    end
    # local mass
    Me = N' * W * N   
...
function GetLocalTraction(Coord_E, Q1d, edge, Pressure)
    ...
    if edge == "bottom"
        Nhat = GetNodalBasis(q1, m1)
        N = reshape(Nhat,2*Q1d,:)
        # get (x,y,z) on E, detJ,...
        # X are in E
        ...
        for i=1:Q1d
            g = Pressure(x[i], y[i])
            tt[2*i-1:2*i, 1] = [0.;-1.] * g * w[i]
        end
        # traction \int{u' * g n}
        t = N' * tt
        
    elseif edge == "top"
        Nhat = GetNodalBasis(q1, p1)
        N = reshape(Nhat,2*Q1d,:)
        # get (x,y,z) on E, detJ,...
        # X are in E
    ...
    elseif edge == "left"
        ...
        for i=1:Q1d
            g = Pressure(x[i], y[i])
            tt[2*i-1:2*i, 1] = [-1.;0.] * g * w[i]
        end
        # traction \int{u' * g n}
        t = N' * tt
        
    elseif edge == "right"
        Nhat = GetNodalBasis(p1, q1)
        N = reshape(Nhat,2*Q1d,:)
        # get (x,y,z) on E, detJ,...
        # X are in E
...
function GetMixedFESol(M, B, Fp, T)
    ...
    U = K \ F
    uh = spzeros(global_dof,1)
    ph = spzeros(num_elem,1)
    for i=1:global_dof
        uh[i] = U[i,1]
    end
...
function GetLocalMat(Coord_E, Q1d)
    ...
    for i=1:m
        JJ = J[:,i,:]
        W[2*i-1:2*i, 2*i-1:2*i]= JJ' * JJ *w[i] /detJ[i]
    end 
...
function ComputeL2Error(Q1d, Coord_Ns, LM, IENn, uh, ph, uexact, pexact)
    ...
    for e=1:num_elem
        ...
        # updated weight
        w_detJ = w .* detJ
        Piola_map = zeros(2*num_qpts,2*num_qpts)
        for i=1:num_qpts
            JJ = J[:,i,:]
            Piola_map[2*i-1:2*i, 2*i-1:2*i]= JJ /detJ[i]
        end
...
println("-------------------------------------------------------------")
println("Results for BDM with $mesh mesh and aspect ratio $aspect_ratio")
println("-------------------------------------------------------------")
println(df)
...
if (parsed_args["plot_mesh"])
    ...
    name1 = "BDM-$mesh-mesh-$aspect_ratio.png"
    savefig(name1)
end